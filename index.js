...
db.run('CREATE TABLE IF NOT EXISTS users (id INTEGER PRIMARY KEY, sccode TEXT, schoolcode TEXT, schoolname TEXT)', (err) => {
    if (err) {
...
client.on(discord.Events.MessageCreate, async message => {
    ...
    if (message.content.startsWith("!school")) {
        ...
        if (command[1] === undefined) {
            db.get('SELECT * FROM users WHERE id = ?', [message.member.id], async (err, row) => {
                if (!row) {
            ...
            res[1].row.forEach(school => {
                select.addOptions(
                    new discord.StringSelectMenuOptionBuilder()
                        ...
                        .setValue(school.ATPT_OFCDC_SC_CODE + " " + school.SD_SCHUL_CODE + " " + school.SCHUL_NM)
                )
            });
...
client.on(discord.Events.InteractionCreate, interaction => {
    ...
    if (interaction.customId === 'schoolSelect') {
        ...
        db.run('INSERT OR REPLACE INTO users (id, sccode, schoolcode, schoolname) VALUES (?, ?, ?, ?)', [interaction.user.id, school[0], school[1], school[2]], (err) => {
            if (err) {
                console.error(err.message);
